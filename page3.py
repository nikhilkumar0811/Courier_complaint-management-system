...
def add():
    ...
    for i in range(len(button)):
        Button(add_frame, text=button[i], command=function[i], height=1, width=20).pack(
...
def search():
    ...
    for i in range(len(button)):
        Button(search_frame, text=button[i], command=function[i], height=1, width=20).pack(
            side=TOP, expand=YES, padx=20, pady=30)
...
def delete():
    ...
    for i in range(len(button)):
        Button(delete_frame, text=button[i], command=function[i], height=1, width=20).pack(
            side=TOP, expand=YES, padx=20, pady=30)
...
def modify():
    ...
    for i in range(len(button)):
        Button(modify_frame, text=button[i], command=function[i], height=1, width=20).pack(
            side=TOP, expand=YES, padx=20, pady=30)
...
def gett():
    ...
    command = 'case_insert1.py '+v1.get()+" "+v2.get()+" "+v3.get()+" "+v4.get() + \
        " "+v5.get()+" "+v6.get()+" "+v7.get()

    lbl = Label(add_patient_frame, text='Added Successfully')
    lbl.pack()
    os.system(command)
    print(command)
...
def get2():
    command = 'python case_search_sec.py '+" "+vk3.get()
    output = subprocess.check_output(command, shell=True)
    lbl['text'] = output.strip()
    os.system(command)
    print(command)
...
frame = add_frame = search_frame = delete_frame = modify_frame = Frame(
    root, bg='blue')
...
modify_patient_frame = Frame(root)
lbl = Label(root, 
		 text="Courier Complaint Management System",
		 fg = "light blue",
		 bg = "dark blue",
